...
import { showMessage } from '@root/@fuse/core/FuseMessage/store/fuseMessageSlice';
import { FuseSettingsConfigType, FuseThemesType, FuseThemeType } from '@root/@fuse/core/FuseSettings/FuseSettings';
import {
	defaultSettings,
	defaultThemeOptions,
...
} from '@root/@fuse/default-settings';
import themeLayoutConfigs from '@root/@fuse/theme-layouts/themeLayoutConfigs';
...
import { darkPaletteText, lightPaletteText } from '@root/app/configs/theme/themesConfig';
import { resetUser, setUser, setUserSettings } from '@root/app/features/user/store/userSlice';
import _ from '@root/app/libs/@lodash';

type AppRootStateType = RootStateType<settingsSliceType>;

export const changeFuseTheme = (theme: FuseThemesType): AppThunk<void> => {
...
const getMainTheme = appSelector((state: AppRootStateType) => {
	return state?.fuseSettings?.current?.theme?.main;
});
